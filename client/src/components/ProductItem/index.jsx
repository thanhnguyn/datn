...
const ProductItem = (props) => {
    ...
    const addToCart = (product, userId, quantity) => {
        ...
        if (props?.item?.productRam?.length !== 0 || props?.item?.productWeight?.length !== 0 || props?.item?.size?.length !== 0) {
            setIsShowTabs(true);
        } else {
                    ...
                    <div className='flex items-center justify-center absolute top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.7)] z-60 p-3 gap-2'>
                        {
                            props?.item?.size?.length !== 0 && props?.item?.size?.map((item, index) => {
                                return (
                                    <span
                                        key={index}
                                        className={`flex items-center justify-center p-1 px-2 bg-[rgba(255,255,255,0.8)] max-w-[65px] h-[30px] rounded-sm cursor-pointer hover:bg-white ${activeTab === index && '!bg-primary text-white'}`}
                        ...
                        }
                        {
                            props?.item?.productWeight?.length !== 0 && props?.item?.productWeight?.map((item, index) => {
                                return (
                                    <span
                                        key={index}
                                        className={`flex items-center justify-center p-1 bg-[rgba(255,255,255,0.8)] max-w-[65px] h-[30px]  rounded-sm cursor-pointer hover:bg-white ${activeTab === index && '!bg-primary text-white'}`}
                                        onClick={() => handleClickActiveTab(index, item)}
                                    >
                                        {item}
                                    </span>
